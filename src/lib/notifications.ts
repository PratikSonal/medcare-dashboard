...
export async function showWelcomeNotification(userName: string) {
  ...
  if (!granted) return;
  setTimeout(() => {
    showLocalNotification('Welcome back to MedCare!', `Good to see you, ${userName}. You have 2 critical alerts today.`);
  }, 1500);
}
...
export async function showPatientAlertNotification(patientName: string, alertType: string) {
  const granted = await requestNotificationPermission();
  if (!granted) return;
  showLocalNotification(`Patient Alert — ${patientName}`, `${alertType} — Immediate attention required.`);
}

export async function showDailySummaryNotification(totalPatients: number, critical: number) {
  const granted = await requestNotificationPermission();
  if (!granted) return;
  showLocalNotification('Daily Summary — MedCare', `${totalPatients} active patients today. ${critical} require critical attention.`);
}
