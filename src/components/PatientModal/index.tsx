      ...
      >
        ...
        <div className="p-6 border-b border-border-primary shrink-0 relative">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-text-secondary mt-1">{patient.id} · {patient.age}y · {patient.gender} · {patient.bloodGroup}</p>
            ...
            >
              ...
              {'count' in t && t.count! > 0 && (
                <span className={cn(
                  ...
                  tab === t.id ? 'bg-[rgba(255,255,255,0.25)] text-white' : 'bg-bg-tertiary text-text-tertiary'
                )}>
                  {t.count}
                </span>
              )}
        ...
        <div className="overflow-y-auto p-6 flex-1">
          <AnimatePresence mode="wait">
                  ...
                  <div className="flex flex-col gap-[10px]">
                    {[
                      ...
                      <div key={text} className="flex items-center gap-[10px] text-[13px] text-text-secondary">
                        <span className="text-accent-blue">{icon}</span>{text}
                      </div>
                    ))}
                  </div>
                </div>
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-[10px]">
                            <div>
                              ...
                              <p className="text-xs text-text-secondary mt-1">{app.doctor} · {app.department}</p>
                              <p className="text-[11px] text-text-tertiary mt-[2px]">{app.clinicName} · {app.duration} min</p>
                            </div>
                            <span
                          ...
                          {app.notes && (
                            <p className="text-[11px] text-[#f59e0b] mt-2 py-[6px] px-[10px] rounded-[8px] bg-[rgba(245,158,11,0.08)]">⚠️ {app.notes}</p>
                          )}
                        </div>
                      );
                    })}
            ...
            {tab === 'billing' && (
              <motion.div key="billing" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col gap-5">
                {billing ? (
                  <>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-2">
                        {billingRecords.map(r => (
                          <div key={r.id} className="p-[14px] rounded-[14px] bg-bg-tertiary border border-border-primary">
                            <div className="flex items-start justify-between gap-3 mb-[10px]">
                              <div>
                                <p className="text-[13px] font-semibold text-text-primary">{r.procedure}</p>
                                <p className="text-[11px] text-text-tertiary mt-[2px]">
                                  {r.department} · {new Date(r.visitDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                                </p>
                              </div>
                              <span
                                className="text-[11px] font-semibold py-[3px] px-[10px] rounded-[8px] shrink-0"
                                style={{ background: CLAIM_STATUS_COLORS[r.claimStatus].bg, color: CLAIM_STATUS_COLORS[r.claimStatus].color }}
                              >
                                {r.claimStatus}
                              </span>
                            </div>
                            <div className="grid grid-cols-3 gap-2">
                              {[
                                { label: 'Total',       value: `₹${r.totalAmount.toLocaleString('en-IN')}` },
                                { label: 'Insurance',   value: `₹${r.insuranceCovered.toLocaleString('en-IN')}` },
                                { label: 'Patient Due', value: `₹${r.patientDue.toLocaleString('en-IN')}` },
                              ].map(({ label, value }) => (
                                <div key={label}>
                ...
                {prescriptions.length > 0 ? (
                  <div className="flex flex-col gap-[10px]">
                    {prescriptions.map(rx => {
                      ...
                      return (
                        <div key={rx.id} className="p-[14px] rounded-[14px] bg-bg-tertiary border border-border-primary">
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <div>
                              ...
                              <p className="text-xs text-text-secondary mt-[3px]">{rx.frequency} · {rx.duration}</p>
                            </div>
                            <span
                              className="text-[11px] font-semibold py-[3px] px-[10px] rounded-[8px] shrink-0"
                              style={{ background: sc.bg, color: sc.color }}
                          ...
                          <div className="flex items-center justify-between flex-wrap gap-2">
                            <p className="text-[11px] text-text-tertiary">
                              {rx.prescribedBy} · {new Date(rx.prescribedDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </p>
                            {rx.refillsLeft !== undefined && rx.status === 'Active' && (
                              <span className={cn(
                                'text-[11px] py-[2px] px-2 rounded-[6px]',
                                rx.refillsLeft > 0 ? 'bg-[rgba(60,131,246,0.1)] text-accent-blue' : 'bg-[rgba(245,158,11,0.1)] text-[#f59e0b]'
                              )}>
                          ...
                          {rx.notes && (
                            <p className="text-[11px] text-[#f59e0b] mt-[6px] py-[6px] px-[10px] rounded-[8px] bg-[rgba(245,158,11,0.08)]">⚠️ {rx.notes}</p>
                          )}
                        </div>
                      );
                    })}
