...
import { DEPARTMENTS, DOCTORS, BLOOD_GROUPS, STATUSES, STATUS_COLORS, STEPS, defaultForm } from './constants';
...
const Field = ({ label, error, children }: { label: string; error?: boolean; children: React.ReactNode }) => {
  return (
    <div>
      <label className={cn('block text-[11px] font-semibold uppercase tracking-[0.04em] mb-[6px]', error ? 'text-[#ef4444]' : 'text-text-tertiary')}>
        {label}{error && <span className="text-[#ef4444]"> *</span>}
      </label>
      {children}
    </div>
  );
...
export function AddPatientModal({ onClose }: { onClose: () => void }) {
  ...
  const inputCls = (field: keyof FormData) => cn(
        ...
        <div className="py-6 px-6 pb-5 border-b border-border-primary shrink-0">
          <div className="flex items-start justify-between mb-5">
            <div>
              ...
              <p className="text-[13px] text-text-secondary mt-[2px]">Step {step + 1} of 3 — {STEPS[step].label} Info</p>
            </div>
            <button onClick={onClose} className="p-[6px] rounded-[10px] bg-bg-tertiary border-0 cursor-pointer text-text-tertiary flex">
              <X size={16} />
            </button>
          </div>
                ...
                <div key={i} className={cn('flex items-center', i < STEPS.length - 1 && 'flex-1')}>
                  <div className="flex flex-col items-center gap-1">
                    <div className={cn(
                      ...
                      done   ? 'bg-[#0ea5e9] border-2 border-[#0ea5e9]' :
                      active ? 'bg-[rgba(60,131,246,0.12)] border-2 border-[#3c83f6]' :
                    ...
                    <span className={cn('text-[10px] font-semibold tracking-[0.03em]', active ? 'text-[#3c83f6]' : done ? 'text-[#0ea5e9]' : 'text-text-tertiary')}>
                      {s.label}
                    </span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={cn('flex-1 h-[2px] mb-[18px] mx-[6px] transition-[background] duration-[250ms]', step > i ? 'bg-[#0ea5e9]' : 'bg-border-primary')} />
                  )}
                </div>
              );
            })}
            ...
            {step === 0 && (
              <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.18 }}
                ...
                <Field label="Full Name" error={errors.name}>
                  <input value={form.name} onChange={set('name')} placeholder="e.g. Riya Mehta" className={inputCls('name')} />
                </Field>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Age" error={errors.age}>
                    <input type="number" min={1} max={120} value={form.age} onChange={set('age')} placeholder="35" className={inputCls('age')} />
                  </Field>
                  <Field label="Gender">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Phone" error={errors.phone}>
                    <input value={form.phone} onChange={set('phone')} placeholder="+91 98765 12345" className={inputCls('phone')} />
                  </Field>
                </div>
                <Field label="Email">
                  <input type="email" value={form.email} onChange={set('email')} placeholder="patient@email.com" className={inputCls('email')} />
                </Field>
                <Field label="Address">
                  <input value={form.address} onChange={set('address')} placeholder="Street, City" className={inputCls('address')} />
                </Field>
              </motion.div>
            )}
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.18 }}
                    ...
                    {STATUSES.map(s => (
                      <button
                        ...
                        style={{
                          ...
                          background: form.status === s ? `${STATUS_COLORS[s]}18` : 'var(--bg-secondary)',
                ...
                <Field label="Diagnosis" error={errors.diagnosis}>
                  <input value={form.diagnosis} onChange={set('diagnosis')} placeholder="e.g. Type 2 Diabetes" className={inputCls('diagnosis')} />
                </Field>
                <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Admission Date">
                    <input type="date" value={form.admissionDate} onChange={set('admissionDate')} className={inputCls('admissionDate')} />
                  </Field>
                  <Field label="Tags (comma-separated)">
                    <input value={form.tags} onChange={set('tags')} placeholder="Diabetic, High BP" className={inputCls('tags')} />
                  </Field>
                </div>
              </motion.div>
            )}
              ...
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.18 }}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Heart Rate (bpm)" error={errors.heartRate}>
                    <input type="number" value={form.heartRate} onChange={set('heartRate')} placeholder="72" className={inputCls('heartRate')} />
                  </Field>
                  <Field label="Blood Pressure" error={errors.bloodPressure}>
                    <input value={form.bloodPressure} onChange={set('bloodPressure')} placeholder="120/80" className={inputCls('bloodPressure')} />
                  </Field>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Temperature (°F)" error={errors.temperature}>
                    <input type="number" step="0.1" value={form.temperature} onChange={set('temperature')} placeholder="98.6" className={inputCls('temperature')} />
                  </Field>
                  <Field label="Oxygen Saturation (%)" error={errors.oxygenSat}>
                    <input type="number" min={0} max={100} value={form.oxygenSat} onChange={set('oxygenSat')} placeholder="98" className={inputCls('oxygenSat')} />
                  </Field>
                </div>
                <Field label="Weight (kg)" error={errors.weight}>
                  <input type="number" value={form.weight} onChange={set('weight')} placeholder="70"
                    className={cn(inputCls('weight'), 'max-w-[calc(50%-8px)]')} />
                </Field>
                <div className="py-3 px-4 bg-bg-tertiary rounded-[12px]">
                  <p className="text-xs text-text-secondary">
                    Patient ID <strong className="text-text-primary">{nextId}</strong> will be assigned on creation.
                  </p>
                </div>
              </motion.div>
            )}
