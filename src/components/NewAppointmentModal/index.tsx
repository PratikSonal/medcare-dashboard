...
import { TSTART, TEND, TDUR, SLOTS, TYPES, DURATIONS, TIMELINE_HOURS, TIMELINE_LEGEND } from './constants';
import { t2m, toLeft, toWidth, minToTime, getConflict, validateForm, buildAppointment } from './helpers';
...
const TrackRow = ({ label, busy, color, colorRgb, formTime, duration, hasConflict, onTimelineClick, onBlockHover }: TrackRowProps) => (
  <div className="mb-2">
    ...
    >
      {busy.map(a => (
        <div
          key={a.id}
          onMouseEnter={e => onBlockHover({ x: e.clientX, y: e.clientY, text: `${a.patientName} · ${a.type} · ${a.duration}min` })}
      ...
      {formTime && (
        <div
          ...
          style={{
            left: toLeft(t2m(formTime)), width: toWidth(duration),
        ...
        <div className="overflow-y-auto flex-1 p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <select value={form.type} onChange={e => up('type', e.target.value as FormState['type'])} className={fieldCls('type')}>
          ...
          <div className="mb-5">
            ...
            {form.doctor
              ? <TrackRow label={form.doctor} busy={docBusy} color="#ef4444" colorRgb="239,68,68" formTime={form.time} duration={form.duration} hasConflict={hasConflict} onTimelineClick={handleTimelineClick} onBlockHover={setHoveredBlock} />
            ...
            {form.patientId
              ? <TrackRow label={patients.find(p => p.id === form.patientId)?.name ?? 'Patient'} busy={patBusy} color="#7c3bed" colorRgb="124,59,237" formTime={form.time} duration={form.duration} hasConflict={hasConflict} onTimelineClick={handleTimelineClick} onBlockHover={setHoveredBlock} />
