...
export default function AppointmentsPage() {
  ...
  const doctors = [...new Set(mockAppointments.map(a => a.doctor))];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '24px' }}>
              ...
              {weekDays.map((day, i) => {
                ...
                return (
                  <motion.button key={key} onClick={() => setSelectedDate(day)}
                    ...
                    {appCount > 0 && (
                      <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: isSelected ? 'var(--accent-blue)' : 'var(--accent-cyan)' }} />
                    )}
                  </motion.button>
                );
              })}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
            ...
            {['09:00', '09:30', '10:00', '11:00', '11:30', '14:00', '14:30', '15:00'].map(slot => {
              const app = todayAll.find(a => a.time === slot);
              return (
                <div key={slot} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 0', borderBottom: '1px solid var(--border-primary)' }}>
