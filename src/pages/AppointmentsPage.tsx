      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "24px" }}>
        ...
        <div>
                ...
                return (
                  <motion.button
                    ...
                    className={cn(
                      ...
                      isSelected ? "border-accent-blue bg-[rgba(60,131,246,0.15)]" : "border-transparent bg-bg-tertiary"
                    )}
                  >
                    ...
                    {appCount > 0 && (
                      <span
                        ...
                        style={{
                          background: isSelected ? "#3c83f6" : appCount >= 6 ? "#ef4444" : appCount >= 4 ? "#f59e0b" : "#0ea5e9",
                        }}
                      >
                        {appCount}
                      </span>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={dateKey + filterStatus} variants={container} initial="hidden" animate="show" className="flex flex-col gap-[10px]">
              {todayApps.length === 0 ? (
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          ...
                          {(app.status === "Pending" || app.status === "Confirmed") && (
                            <div className="flex gap-1">
                              {app.status === "Pending" && (
                                <button
                                  onClick={(e) => { e.stopPropagation(); dispatch(updateAppointmentStatus({ id: app.id, status: "Confirmed" })); dispatch(addToast({ message: `Appointment for ${app.patientName} confirmed.`, type: "success" })); }}
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); dispatch(updateAppointmentStatus({ id: app.id, status: "No-Show" })); dispatch(addToast({ message: `${app.patientName} marked as no-show.`, type: "info" })); }}
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); dispatch(updateAppointmentStatus({ id: app.id, status: "Cancelled" })); dispatch(addToast({ message: `Appointment for ${app.patientName} cancelled.`, type: "error" })); }}
        ...
        <div className="flex flex-col gap-4">
          {/* Doctor schedules */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="glass-card rounded-20 p-5">
            ...
            <div className="flex flex-col gap-[10px]">
              {doctors.map((doc) => {
                ...
                return (
                  <div key={doc} className="flex items-center justify-between py-[10px] px-3 rounded-[12px] bg-bg-tertiary">
                    ...
                    <div className="flex items-center gap-[6px]">
                      <div className="w-[60px] h-1 rounded-full bg-border-primary overflow-hidden">
                        <div
                          ...
                          style={{ width: `${docApps.length > 0 ? (confirmed / docApps.length) * 100 : 0}%`, transition: "width 600ms ease" }}
                        />
                      </div>
          ...
          {/* Action Required */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="glass-card rounded-20 p-5">
            ...
            <div className="flex flex-col gap-2">
              {todayAll.filter((a) => !a.intakeComplete || !a.insuranceVerified).map((app) => (
                <div key={app.id} className="p-3 rounded-[12px] bg-[rgba(245,158,11,0.06)] border border-[rgba(245,158,11,0.2)]">
                  ...
                  <div className="flex gap-[6px] flex-wrap">
                    {!app.intakeComplete && (
                      <button
                        onClick={() => { dispatch(updateAppointmentChecks({ id: app.id, intakeComplete: true })); dispatch(addToast({ message: `Intake completed for ${app.patientName}.`, type: "success" })); }}
                    ...
                    {!app.insuranceVerified && (
                      <button
                        onClick={() => { dispatch(updateAppointmentChecks({ id: app.id, insuranceVerified: true })); dispatch(addToast({ message: `Insurance verified for ${app.patientName}.`, type: "success" })); }}
          ...
          {/* Time Slots */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="glass-card rounded-20 p-5">
            <h3 className="text-[15px] font-semibold text-text-primary mb-4 flex items-center gap-2">
            ...
            {todayAll.length === 0 ? (
              ...
              todayAll.map((app, i) => (
                <div key={app.id} className={cn("flex items-center gap-[10px] py-[7px]", i < todayAll.length - 1 && "border-b border-border-primary")}>
            ...
            >
              <div className="p-6 bg-bg-tertiary border-b border-border-primary">
                <div className="flex items-center gap-[14px]">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-[18px] font-bold text-text-primary">{selectedApp.patientName}</h2>
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      <span className="text-xs text-text-secondary">DOB: {selectedApp.dob}</span>
                  ...
                  <span
                    ...
                    style={{ background: STATUS_CONFIG[selectedApp.status].bg, color: STATUS_CONFIG[selectedApp.status].color }}
                  >
              ...
              <div className="p-6 flex flex-col gap-4">
                ...
                <div className="flex gap-3">
                  <div className={cn("flex-1 p-[14px] rounded-[14px] text-center", selectedApp.intakeComplete ? "bg-[rgba(14,165,233,0.08)] border border-[rgba(14,165,233,0.2)]" : "bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.2)]")}>
                    <p className="text-[20px] mb-1">{selectedApp.intakeComplete ? "✅" : "⏳"}</p>
                    <p className={cn("text-xs font-semibold", selectedApp.intakeComplete ? "text-[#0ea5e9]" : "text-[#f59e0b]")}>Patient Intake</p>
                    <p className="text-[11px] text-text-tertiary mt-[2px]">{selectedApp.intakeComplete ? "Complete" : "Pending"}</p>
                  </div>
                  <div className={cn("flex-1 p-[14px] rounded-[14px] text-center", selectedApp.insuranceVerified ? "bg-[rgba(14,165,233,0.08)] border border-[rgba(14,165,233,0.2)]" : "bg-[rgba(239,68,68,0.08)] border border-[rgba(239,68,68,0.2)]")}>
                    <p className="text-[20px] mb-1">{selectedApp.insuranceVerified ? "✅" : "❌"}</p>
                    <p className={cn("text-xs font-semibold", selectedApp.insuranceVerified ? "text-[#0ea5e9]" : "text-[#ef4444]")}>Insurance</p>
                    <p className="text-[11px] text-text-tertiary mt-[2px]">{selectedApp.insuranceVerified ? "Verified" : "Not Verified"}</p>
                  </div>
                </div>
                ...
                {selectedApp.notes && (
                  <div className="p-[14px] rounded-[14px] bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.2)]">
                    ...
                    <p className="text-[13px] text-text-secondary leading-[1.6]">{selectedApp.notes}</p>
                  </div>
                )}
