...
export const TrendsRow = () => {
  ...
  const chartData = useMemo(
    () => chartPeriod === "3M" ? metricsData.slice(-3) : chartPeriod === "6M" ? metricsData.slice(-6) : metricsData,
    [chartPeriod],
  );
      ...
      <motion.div variants={item} className="glass-card rounded-20 p-6">
            ...
            {(["3M", "6M", "All"] as const).map((p) => (
              <button
                ...
                className={cn(
                  ...
                  chartPeriod === p ? "bg-accent-blue text-white" : "bg-bg-tertiary text-text-secondary",
                )}
              >
                {p}
              </button>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <Tooltip
              contentStyle={{ background: "var(--bg-card)", border: "1px solid var(--border-primary)", borderRadius: "12px", color: "var(--text-primary)", fontFamily: "Poppins", fontSize: "12px" }}
