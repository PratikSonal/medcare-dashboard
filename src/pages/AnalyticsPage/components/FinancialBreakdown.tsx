  ...
  const { topProcedures, providerData } =
    useMemo(() => {
      ...
      records.forEach(record => {
        const name = PROVIDER_SHORT[record.insuranceProvider] || record.insuranceProvider;
        if (!providerMap[name]) providerMap[name] = { total: 0, covered: 0, due: 0, claims: 0 };
        providerMap[name].total += record.totalAmount;
        providerMap[name].covered += record.insuranceCovered;
        providerMap[name].due += record.patientDue;
          ...
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              {providerData.map((provider, i) => (
                ...
                >
                  <td className="px-2 py-[10px] font-medium text-text-primary">{provider.name}</td>
                  <td className="px-2 py-[10px] text-text-secondary">{formatCompact(provider.total)}</td>
                  <td className="px-2 py-[10px]">
                    <div className="w-16 h-[5px] rounded-full bg-border-primary overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${provider.rate}%`,
                          background: provider.rate > 60 ? "var(--accent-cyan)" : provider.rate > 30 ? "var(--accent-yellow)" : "var(--accent-red)",
                        }}
                      />
                    </div>
                  </td>
                  <td
                    ...
                    style={{ color: provider.rate > 60 ? "var(--accent-cyan)" : provider.rate > 30 ? "var(--accent-yellow)" : "var(--accent-red)" }}
                  >
                    {provider.rate}%
                  </td>
                </motion.tr>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-text-tertiary mt-[2px]">
                    {procedure.department} · {procedure.patientName}
                  </p>
                  <div className="flex items-center gap-2 mt-[6px]">
                    <div className="flex-1 h-1 rounded-full bg-border-primary overflow-hidden">
                      <div
                        className="h-full rounded-full"
                        style={{
                          width: `${Math.round((procedure.totalAmount / topProcedures[0].totalAmount) * 100)}%`,
                          background: PROC_COLORS[i],
                        }}
                      />
                    </div>
                    <span className="text-[11px] font-semibold text-text-primary whitespace-nowrap">
                      ₹{procedure.totalAmount.toLocaleString("en-IN")}
                    </span>
                  </div>
                </div>
              </motion.div>
