...
function PatientCard({ patient, onClick }: { patient: Patient; onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      className={cn('glass-card rounded-20 p-5 cursor-pointer relative overflow-hidden', isCritical && 'border-[rgba(239,68,68,0.4)]')}
    >
      {isCritical && (
        <div className="absolute top-3 right-3 w-2 h-2 rounded-full bg-accent-red" style={{ animation: 'pulse 2s ease-in-out infinite' }} />
      )}
      <div className="absolute inset-0 pointer-events-none" style={{ opacity: 0.15, background: 'var(--gradient-card)' }} />
      <div className="relative">
        <div className="flex items-start gap-3 mb-[14px]">
          <Avatar initials={patient.avatar} size={44} radius="14px" />
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <h3 className="text-sm font-semibold text-text-primary overflow-hidden text-ellipsis whitespace-nowrap">{patient.name}</h3>
              <span
            ...
            <p className="text-[11px] text-text-tertiary mt-[2px]">{patient.id} · {patient.age}y · {patient.gender}</p>
          </div>
        </div>
        <div className="flex flex-col gap-[6px] mb-[14px]">
          <div className="flex items-center gap-2 text-xs text-text-secondary">
            <Activity size={12} className="text-accent-blue shrink-0" />
            <span className="overflow-hidden text-ellipsis whitespace-nowrap">{patient.diagnosis}</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-text-secondary">
            <Heart size={12} className="text-accent-cyan shrink-0" />
            <span className="overflow-hidden text-ellipsis whitespace-nowrap">{patient.department}</span>
          </div>
        </div>
        <div className="flex items-center justify-between pt-3 border-t border-border-primary">
          <span className="text-[11px] text-text-tertiary overflow-hidden text-ellipsis whitespace-nowrap flex-1">{patient.doctor}</span>
          <span className="text-[11px] text-text-tertiary shrink-0 ml-2">{formatDate(patient.lastVisit)}</span>
        </div>
        {patient.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-[10px]">
            {patient.tags.slice(0, 2).map(tag => (
              <span key={tag} className="text-[10px] py-[2px] px-2 rounded-full bg-bg-tertiary text-text-tertiary">{tag}</span>
            ))}
            {patient.tags.length > 2 && (
              <span className="text-[10px] py-[2px] px-2 rounded-full bg-bg-tertiary text-text-tertiary">+{patient.tags.length - 2}</span>
            )}
          </div>
        )}
      </div>
...
function PatientListRow({ patient, onClick }: { patient: Patient; onClick: () => void }) {
  return (
    ...
    >
      ...
      <td className="py-[14px] px-4 text-[13px] text-text-secondary">{patient.age}y · {patient.gender}</td>
  ...
  return (
    <div className="max-w-[1280px] mx-auto">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1">{filteredPatients.length} of {patients.length} patients</p>
          </div>
      ...
      >
        ...
        {hasActiveFilters && (
          <Button variant="ghost" size="sm" onClick={() => dispatch(clearFilters())} className="flex items-center gap-[6px]">
            <X size={14} /> Clear
          </Button>
        )}
      </motion.div>
          ...
          >
            <div className="grid grid-cols-2 gap-6">
              {[
                ...
                <div key={label}>
                  <p className="text-[11px] font-semibold text-text-tertiary uppercase tracking-[0.05em] mb-3">{label}</p>
                    ...
                    {items.map(s => (
                      <button
                        ...
                        className={cn(
                          ...
                          current === s ? 'bg-accent-blue text-white' : 'bg-bg-tertiary text-text-secondary'
                        )}
                      >
                        {s}
                      </button>
      ...
      <AnimatePresence mode="wait">
        {viewMode === 'grid' ? (
          ...
          <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="glass-card rounded-20 overflow-hidden">
            <table className="w-full border-collapse">
              <thead className="bg-bg-tertiary border-b border-border-primary">
                <tr>
                  {['Patient', 'Age / Gender', 'Diagnosis', 'Department', 'Doctor', 'Status', 'Last Visit'].map(h => (
                    <th key={h} className="text-left py-3 px-4 text-[11px] font-semibold text-text-tertiary uppercase tracking-[0.05em]">{h}</th>
                  ))}
                </tr>
              </thead>
      ...
      {filteredPatients.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
          ...
          <Button variant="outline" onClick={() => dispatch(clearFilters())} className="mt-4">Clear Filters</Button>
        </motion.div>
      )}
